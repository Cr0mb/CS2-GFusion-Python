...
def get_vk_code(key_name):
    """Return Windows virtual key code for given key name, supporting common names and F-keys."""
    if not key_name:
        return None
...
class AimbotRCS:
    ...
    def __init__(self, cfg):
        ...
        # Normalize/alias config so toggles reliably work
        self._normalize_cfg()
    ...
    def _normalize_cfg(self):
        """Ensure required config fields exist and map common aliases so toggles take effect."""
        cfg = self.cfg
        # Enabled
        if not hasattr(cfg, 'enabled'):
            for name in ('aim_enabled', 'aimbot_enabled', 'aim'):
                if hasattr(cfg, name):
                    setattr(cfg, 'enabled', bool(getattr(cfg, name)))
                    break
            else:
                setattr(cfg, 'enabled', True)
        # Aim key
        if not hasattr(cfg, 'aim_key'):
            for name in ('aimbot_key', 'aim_hotkey', 'aimkey'):
                if hasattr(cfg, name):
                    setattr(cfg, 'aim_key', getattr(cfg, name))
                    break
            else:
                setattr(cfg, 'aim_key', 'mouse5')
        # FOV
        if not hasattr(cfg, 'FOV'):
            for name in ('aim_fov', 'fov', 'aimbot_fov'):
                if hasattr(cfg, name):
                    setattr(cfg, 'FOV', float(getattr(cfg, name)))
                    break
            else:
                setattr(cfg, 'FOV', 60.0)
        # Team logic
        if not hasattr(cfg, 'DeathMatch'):
            for name in ('deathmatch', 'death_match', 'dm'):
                if hasattr(cfg, name):
                    setattr(cfg, 'DeathMatch', bool(getattr(cfg, name)))
                    break
            else:
                setattr(cfg, 'DeathMatch', False)
        # Entities
        if not hasattr(cfg, 'max_entities'):
            setattr(cfg, 'max_entities', 256)
        # Bones
        if not hasattr(cfg, 'target_bone_name'):
            setattr(cfg, 'target_bone_name', 'head')
        if not hasattr(cfg, 'bone_indices_to_try'):
            # head, chest, neck-ish, pelvis-ish (approx indices vary by game updates)
            setattr(cfg, 'bone_indices_to_try', [6, 18, 5, 4, 3])
        # Prediction
        if not hasattr(cfg, 'enable_velocity_prediction'):
            setattr(cfg, 'enable_velocity_prediction', True)
        if not hasattr(cfg, 'velocity_prediction_factor'):
            setattr(cfg, 'velocity_prediction_factor', 1.0)
        if not hasattr(cfg, 'downward_offset'):
            setattr(cfg, 'downward_offset', 0.0)
        if not hasattr(cfg, 'closest_to_crosshair'):
            setattr(cfg, 'closest_to_crosshair', True)
        # Focus check behavior (default: do not require CS2 focused)
        if not hasattr(cfg, 'require_cs2_focus'):
            setattr(cfg, 'require_cs2_focus', False)
        # Learning
        if not hasattr(cfg, 'enable_learning'):
            setattr(cfg, 'enable_learning', True)
        if not hasattr(cfg, 'learn_dir'):
            setattr(cfg, 'learn_dir', 'learn')
        if not hasattr(cfg, 'learn_max_entries'):
            setattr(cfg, 'learn_max_entries', 150)
        # Mouse recording
        if not hasattr(cfg, 'enable_mouse_recording'):
            setattr(cfg, 'enable_mouse_recording', True)
        if not hasattr(cfg, 'mouse_record_rate'):
            setattr(cfg, 'mouse_record_rate', 125)
        # Humanization & control
        if not hasattr(cfg, 'humanization_enabled'):
            setattr(cfg, 'humanization_enabled', True)
        cfg.sensitivity = float(getattr(cfg, 'sensitivity', 0.022))
        cfg.invert_y = int(getattr(cfg, 'invert_y', -1))
        cfg.max_mouse_move = int(getattr(cfg, 'max_mouse_move', 25))
        cfg.human_blend = float(getattr(cfg, 'human_blend', 0.35))
        if not hasattr(cfg, 'rcs_scale'):
            setattr(cfg, 'rcs_scale', 1.0)
    ...
    def run(self):
        ...
        while not self.cfg.aim_stop:
            ...
            try:
                ...
                # Optionally require CS2 focus (configurable). By default, do NOT require focus.
                if aim_vk is None or (getattr(self.cfg, 'require_cs2_focus', False) and not self.is_cs2_focused()):
                    time.sleep(0.1)
                    continue

...
class SpectatorList:
    ...
    def _get_entity(self, entity_list, handle):
        ...
        if not entry:
            return 0
        return self._safe_read_uint64(entry + 120 * lo)
...
def _scan_world_items_once(handle, base, matrix, overlay, cfg):
    ...
    for i in range(scan_start, scan_end):
        try:
            ...
            ent = sru64(handle, list_entry + (120 * (i & 0x1FF)))
            if not ent:
                continue
            ...
            try:
                ...
                for slot in range(5):
                    try:
                        ...
                        weapon_entity = safe_read_uint64(handle, weapon_entry + (120 * (weapon_index & 0x1FF)))
                        ...
                        if observer_mode in [4, 5, 6]:  # Spectating someone
                            ...
                            if observer_target > 0:
                                ...
                                if target_list_entry:
                                    target_controller = safe_read_uint64(handle, target_list_entry + (120 * (target_index & 0x1FF)))
                                    if target_controller:
                                        ...
                                        if spectated_pawn_handle:
                                            ...
                                            if spectated_list_entry:
                                                spectated_pawn = safe_read_uint64(handle, spectated_list_entry + (120 * (spectated_index & 0x1FF)))
                except:
